...
class LoginActivity : AppCompatActivity() {
    ...
    private fun criarContaProvisoria() {
        val emailParaCriar = "victor_vglr@hotmail.com"
        val senhaParaCriar = "COla47@*"
        val nomeParaCriar = "Victor"

        Toast.makeText(this, "Tentando criar usuário...", Toast.LENGTH_LONG).show()

        repository.createUserOnce(emailParaCriar, senhaParaCriar, nomeParaCriar) { sucesso, msg ->
            if (sucesso) {
    ...
    private fun mostrarDialogoBiometria() {
        ...
        val callback = object : BiometricPrompt.AuthenticationCallback() {
            override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult) {
                ...
                Toast.makeText(applicationContext, "Autenticado via Biometria!", Toast.LENGTH_SHORT).show()
                abrirHome()
            }
